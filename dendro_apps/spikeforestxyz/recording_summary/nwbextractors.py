...
from typing import List, Optional, Literal, Dict, BinaryIO, Any
import warnings
...
def read_file_from_backend(
    *,
    ...
    elif h5py_file is not None:
        open_file = h5py_file
    else:
        raise ValueError("Provide either file_path or file or h5py_file")
...
def read_nwbfile(
    *,
    ...
    if file_path is not None and file is not None:
        raise ValueError("Provide either file_path or file or h5py_file, not both")
    if file_path is not None and h5py_file is not None:
        raise ValueError("Provide either file_path or file or h5py_file, not both")
    if file is not None and h5py_file is not None:
        raise ValueError("Provide either file_path or file or h5py_file, not both")
    if file_path is None and file is None and h5py_file is None:
        raise ValueError("Provide either file_path or file or h5py_file")
...
class NwbRecordingExtractor(BaseRecording):
    ...
    def __init__(
        ...
        *,
        ...
        h5py_file: Any | None = None,  # h5py.File - like
        cache: bool = False,
        storage_options: dict | None = None,
        ...
        if self.stream_mode is None and file is None and h5py_file is None:
        ...
        if use_pynwb:
            ...
            (
                ...
                segment_data,
                times_kwargs,
            ) = self._fetch_recording_segment_info_pynwb(file, h5py_file, cache, load_time_vector, samples_for_rate_estimation)
        else:
            (
                channel_ids,
                sampling_frequency,
                ...
                segment_data,
                times_kwargs,
            ) = self._fetch_recording_segment_info_backend(file, h5py_file, cache, load_time_vector, samples_for_rate_estimation)
        ...
        if file is not None or h5py_file is not None:
    ...
    def _fetch_recording_segment_info_pynwb(self, file, h5py_file, cache, load_time_vector, samples_for_rate_estimation):
    ...
    def _fetch_recording_segment_info_backend(self, file, h5py_file, cache, load_time_vector, samples_for_rate_estimation):
        open_file = read_file_from_backend(
            backend=self.backend,
            file_path=self.file_path,
            file=file,
...
class NwbSortingExtractor(BaseSorting):
    ...
    def __init__(
        ...
        *,
        ...
        if self.file_path is not None and self.h5py_file is not None:
            raise ValueError("Provide either file_path or h5py_file, not both")
        if self.file_path is None and self.h5py_file is None:
            raise ValueError("Provide either file_path or h5py_file")

        if self.stream_mode is None and file_path is not None:
            self.backend = _get_backend_from_local_file(file_path)
        else:
            if self.stream_mode == "zarr":
                self.backend = "zarr"
...
def read_nwb(*, file_path, h5py_file, load_recording=True, load_sorting=False, electrical_series_path=None):
    ...
    outputs = ()
    if load_recording:
        rec = read_nwb_recording(file_path=file_path, h5py_file=h5py_file, electrical_series_path=electrical_series_path)
        outputs = outputs + (rec,)
    if load_sorting:
        sorting = read_nwb_sorting(file_path=file_path, h5py_file=h5py_file, electrical_series_path=electrical_series_path)
